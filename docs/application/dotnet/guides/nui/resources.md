...
This tutorial describes the use of image resources in NUII.

## Image handling methods

The two basic methods of handle image resources in NUI are as follows:

1. Visuals are the main building block for UI components and provide reusable rendering logic that is controlled using properties. For more information, [Visuals tutorial](visuals.md) page.
    ```
...
Visual creation automatically determines whther the image is N_PATCH, SVG or GIF, which is not a regular image from the image data.
...
Synchronous loading has niche uses, for example, ensure image has loded before continuing to exceute application code,
...
To support this, DALi can resize an imgae at load time so that its in-memory copy uses less space and its visual quality benefits from being prefltered.
...
| Fitting mode 	| Suggesting use case |
| ------------- | ------------------ |
